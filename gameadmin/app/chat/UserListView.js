Ext.define('app.chat.UserListView', {
    ...
    ,
    doSetHeadImg: function () {
        Ext.MessageBox.confirm('批量更新机器人头像', '请确保img/robot目录下有头像图片，确定？', function (txt) {
            if ('yes' == txt) {
                jsonRequest.execute({
                    ...
                    if(code ==200) {
                        Ext.MessageBox.alert("提示","批量更新机器人头像成功！");
                       this.store.reload();
                    }
                },this);
            }
    ...
    doSetRobotMoney: function () {
        Ext.MessageBox.confirm('批量更新机器人余额', '批量更新机器人余额为100000，确定？', function (txt) {
            if ('yes' == txt) {
                jsonRequest.execute({
                    service:this.listService,
                    method:'setRobotMoney',
                    parameters:{
                    }
                },function(code,msg,json){
                    if(code ==200) {
                        Ext.MessageBox.alert("提示","批量更新机器人余额成功！");
                       this.store.reload();
                    }
                },this);
            }
    ...
    ,
    doAddrobot: function () {
        Ext.MessageBox.confirm('批量增加100机器人', '批量增加100机器人，确定？', function (txt) {
            if ('yes' == txt) {
                jsonRequest.execute({
                    service:this.listService,
                    method:'addrobot',
                    ...
                    if(code ==200) {
                        Ext.MessageBox.alert("提示","批量增加100机器人成功！");
                       this.store.reload();
                    }
                },this);
            }
