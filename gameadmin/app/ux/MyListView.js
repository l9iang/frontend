Ext.define('app.ux.MyListView', {
    ...
    initComponent: function () {
        ...
        var queryItems = [];
        this.dicColumns = [];
        this.columnCount=this.schema.columnCount||3;
        this.layout.columns=this.columnCount;
    ...
    getForm: function () {
        ...
        if (!form) {
            form = Ext.create('app.ux.MyFormPanel', {
                schema: this.schema,
                columnCount:this.schema.columnCount,
    ...
    doUpdateByBatch: function () {
        Ext.MessageBox.prompt('批量分配机器人', '请输入每个房间分配机器人数量，缺省为8,提交后等待数秒获得处理通知', function (btn, text)  {
           if ('ok' == btn) 
            {
                jsonRequest.execute({
                    service:this.listService,
                    method:'updateByBatch',
                    parameters:{
                    	num:text,
                    }
                },function(code,msg,json){
                    if(code ==200) {
                        Ext.MessageBox.alert("提示","批量分配机器人成功，点击刷新改页面查看最新情况");
                       this.store.reload();
                    }
                    else
                    	{
                    	   Ext.MessageBox.alert("提示","分配失败，稍后再试");
    ...
    doUpdateAddByBatch: function () {
        Ext.MessageBox.prompt('批量增加机器人', '请输入每个房间增加机器人数量，缺省为8,提交后等待数秒获得处理通知', function (btn, text)  {
            if ('ok' == btn) 
            {
                jsonRequest.execute({
                    service:this.listService,
                    method:'updateAddByBatch',
                    parameters:{
                    	num:text,
                    }
                },function(code,msg,json){
                    if(code ==200) {
                        Ext.MessageBox.alert("提示","批量增加机器人成功，点击刷新改页面查看最新情况");
                       this.store.reload();
                    }
                    else
                    	{
                    	   Ext.MessageBox.alert("提示","增加失败，稍后再试");
    ...
    doUpdeteRoomByBatch: function () {
        Ext.MessageBox.prompt('批量更新房间虚拟人数', '请输入每个房间分配增加虚拟人数，缺省为20,提交后等待数秒获得处理通知', function (btn, text)  {
          if ('ok' == btn) 
            {
                jsonRequest.execute({
                    service:this.listService,
                    method:'updeteRoomByBatch',
                    parameters:{
                    	num:text,
                    }
                },function(code,msg,json){
                    if(code ==200) {
                        Ext.MessageBox.alert("提示","批量分配成功，点击刷新改页面查看最新情况");
                       this.store.reload();
                    }
                    else
                    	{
